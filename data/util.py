...
def unmold_mask(mask, bbox, image_shape, threshold=0.5):
    ...
    mask(np.array): [height, width] of type float. A small, typically 28x28 mask.
    bbox(np.array): [x1, y1, x2, y2]. The box to fit the mask in.
    image_shape: the size of full image where the mask is to be embedded
    threshold: the value for masking, default is 0.5
    ...
    # the result is [0,1]
    print('the size of mask is', y2-y1,',', x2-x1)
    np.save(uuid.uuid4().hex, mask)

    if y2-y1 == 0:
        if y2+1>image_shape[0]:
            y1 -= 1
        else:
            y2 += 1
    
    if x2-x1 == 0:
        if x2+1>image_shape[1]:
            x1 -= 1
        else:
            x2 += 1

    mask = sktsf.resize(mask, (y2 - y1, x2 - x1), order=1, mode='constant', anti_aliasing=False).astype(np.float32)
